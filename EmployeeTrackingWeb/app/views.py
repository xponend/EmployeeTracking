...
def org_login_required(function):
    def wrapper(request, *args, **kw):
        if "logged_in" in request.session:
            ...
            messages.error(
                request, "Logout Request/ Unauthorized Request, Please login!"
            )
...
def contact(request):
    if request.method == "POST":
        ...
        subject = "EmployeeTrackingDesk - New Enquiry"
        ...
        send_mail(
            subject,
            "YOUR QUERY WILL BE PROCESSED! WITHIN 24 HOURS",
            email_from,
            [cemail],
        )
...
def org_login(request):
    if request.method == "POST":
        ...
        org_details = Organization.objects.filter(
            o_email=o_email, o_password=o_pass
        ).values()
        if org_details:
...
def user_login(request):
    if request.method == "POST":
        ...
        user_details = Employee.objects.filter(
            e_email=e_email, e_password=e_pass
        ).values()
        if user_details:
            request.session["logged_in"] = True
...
def org_register(request):
    if request.method == "POST":
        ...
        if password1 == password2:
            ...
            subject = "EmployeeTrackingDesk - OTP Verification"
            message = f"Hi {o_name}, thank you for registering in EmployeeTrackingDesk . Your One Time Password (OTP) for verfication is {otp}"
            email_from = settings.EMAIL_HOST_USER
            recipient_list = [
                o_email,
            ]
...
def org_change_password(request):
    if request.method == "POST":
        ...
        org_details = Organization.objects.filter(
            o_email=o_email, o_password=oldPwd, pk=o_id
        ).update(o_password=newPwd)
        if org_details:
            subject = "EmployeeTrackingDesk - Password Changed"
            message = (
                f"Hi, your password was changed successfully! From EmployeeTrackingDesk"
            )
            email_from = settings.EMAIL_HOST_USER
            recipient_list = [
                o_email,
            ...
            subject = "EmployeeTrackingDesk - Notifications"
            message = f"Hi, there was attempt to change your password! From EmployeeTrackingDesk"
            email_from = settings.EMAIL_HOST_USER
            recipient_list = [
                o_email,
            ]
...
def user_change_password(request):
    if request.method == "POST":
        ...
        emp_details = Employee.objects.filter(
            e_email=u_email, e_password=oldPwd, pk=u_id
        ).update(e_password=newPwd)
        if emp_details:
            subject = "EmployeeTrackingDesk - Password Changed"
            message = (
                f"Hi, your password was changed successfully! From EmployeeTrackingDesk"
...
def org_forgot_password(request):
    if request.method == "POST":
        ...
        if org_details:
            ...
            subject = "EmployeeTrackingDesk - OTP Verification for Forgot Password"
...
def org_forgot_password_change_password(request):
    if request.method == "POST":
        ...
        if pwd1 == pwd2:
            org_details = Organization.objects.filter(o_email=tempOrgFpEmail).update(
                o_password=pwd1
            )
            if org_details:
                subject = "EmployeeTrackingDesk - Password was Changed"
...
def user_forgot_password_change_password(request):
    if request.method == "POST":
        ...
        pwd2 = request.POST["pwd2"]
        if pwd1 == pwd2:
            emp_details = Employee.objects.filter(e_email=tempEmpFpEmail).update(
                e_password=pwd1
            )
            if emp_details:
                subject = "EmployeeTrackingDesk - Password was Changed"
...
def report_org(request):
    if request.method == "POST":
        ...
        subject = "EmployeeTrackingDesk - New Enquiry"
        message = (
            f"Name : {cname}, Email : {cemail}, Problem : {ptype}, Query : {cquery}"
        )
        email_from = settings.EMAIL_HOST_USER
        recipient_list = [
            "employer@gmail.com",
        ...
        send_mail(
            subject,
            "Your Problem has been recorded. From: EmployeeTrackingDesk",
            email_from,
            [cemail],
        )
...
def add_emp(request):
    if request.method == "POST":
        ...
        if empObj:
            subject = "EmployeeTrackingDesk - Login Info"
            org_name = request.session["o_name"]
            message = f"Name : {e_name}, \n Email : {e_email}, \n Password : {e_password} \n Organization : {org_name}, \n FROM - EmployeeTrackingDesk \n Developed by ATU"
            email_from = settings.EMAIL_HOST_USER
...
def create_proj(request):
    if request.method == "POST":
        ...
        projCheck = Project.objects.filter(
            p_name=p_name, o_id_id=request.session["o_id"]
        )
        if projCheck:
...
def projectwise_task(request, pid):
    if request.method == "GET":
        ...
        count_no_of_completed_tasks = Task.objects.filter(
            o_id_id=o_id, p_id_id=pid, t_status="completed"
        ).count()
        count_no_of_pending_tasks = (
            count_no_of_total_tasks - count_no_of_completed_tasks
        )
        if tasks:
            tasklist = []
            for task in tasks:
...
def user_projectwise_task(request, pid):
    if request.method == "GET":
        ...
        count_no_of_total_tasks = Task.objects.filter(
            o_id_id=o_id, e_id_id=e_id, p_id_id=pid
        ).count()
        count_no_of_completed_tasks = Task.objects.filter(
            o_id_id=o_id, e_id_id=e_id, p_id_id=pid, t_status="completed"
        ).count()
        count_no_of_pending_tasks = (
            count_no_of_total_tasks - count_no_of_completed_tasks
        )
...
def read_meet(request):
    if request.method == "GET":
        meeting_details = Meeting.objects.filter(
            o_id_id=request.session["o_id"]
        ).values()
...
def user_read_meets(request):
    if request.method == "GET":
        ...
        e_id = request.session["u_id"]
        pel_details = (
            Project_Employee_Linker.objects.filter(o_id_id=o_id, e_id_id=e_id)
            .order_by("-id")
...
def assign_proj_emp(request):
    o_id = request.session["o_id"]
    projAssign_details = Project_Employee_Linker.objects.filter(
        o_id_id=o_id
    ).prefetch_related("p_id_id", "e_id_id")
    project_details = Project.objects.filter(o_id_id=o_id).values()
    emp_details = Employee.objects.filter(o_id_id=o_id).values()
    if request.method == "POST":
        p_id = request.POST["p_id"]
        e_id = request.POST["e_id"]
        projAssignCheck = Project_Employee_Linker.objects.filter(
            p_id_id=p_id, e_id_id=e_id, o_id_id=o_id
        )
        if projAssignCheck:
            messages.error(request, "Project already exists!")
            return HttpResponseRedirect("/assign-proj")
        else:
            projAssignCheckObj = Project_Employee_Linker.objects.create(
                e_id_id=e_id, o_id_id=o_id, p_id_id=p_id
            )
            if projAssignCheckObj:
                ...
                if user_details and project_details:
                    subject = "EmployeeTrackingDesk - Project was Assigned to you"
                    message = f"Hi, {s_name} You are asssigned to project {s_p_name} Check on EmployeeTrackingDesk !"
                    email_from = settings.EMAIL_HOST_USER
                    recipient_list = [s_email]
                    send_mail(subject, message, email_from, recipient_list)
                messages.success(
                    request, "Employee was Successfully Assigned with Project!"
                )
                return HttpResponseRedirect("/assign-proj")
            else:
                messages.error(request, "Error was occurred!")
    ...
    return render(
        request,
        "AssignProjEmp.html",
        {"msg1": project_details, "msg2": emp_details, "msg3": projAssign_details},
    )
...
def create_meet(request):
    ...
    if request.method == "POST":
        ...
        m_uuid = uuid.uuid1()
        meetingObj = Meeting.objects.create(
            m_name=m_name,
            m_desc=m_desc,
            m_uuid=m_uuid,
        ...
        if meetingObj:
            pel_details = Project_Employee_Linker.objects.filter(
                o_id_id=o_id, p_id_id=p_id
            ).values_list("e_id_id", flat=True)
            user_details = Employee.objects.filter(id__in=pel_details).values()
            if user_details:
                for ud in user_details:
                    subject = "EmployeeTrackingDesk - New Meeting"
                    s_name = ud["e_name"]
                    s_email = ud["e_email"]
                    message = f"Hi, {s_name} New Meeting for created for you! Details are Meeting Name :{m_name}, Meeting Description: {m_desc}, Date Time: {start_date} {start_time} Check on EmployeeTrackingDesk !"
                    email_from = settings.EMAIL_HOST_USER
                    recipient_list = [s_email]
                    send_mail(subject, message, email_from, recipient_list)
...
def view_emp(request, eid):
    if request.method == "GET":
        ...
        count_no_of_completed_tasks = Task.objects.filter(
            o_id_id=o_id, e_id_id=eid, t_status="completed"
        ).count()
        count_no_of_pending_tasks = (
            count_no_of_total_tasks - count_no_of_completed_tasks
        )
        pel_details = Project_Employee_Linker.objects.filter(
            o_id_id=o_id, e_id_id=eid
        ).values_list("p_id_id", flat=True)
...
def del_emp(request, eid):
    try:
        emp_detail = Employee.objects.filter(
            id=eid, o_id_id=request.session["o_id"]
        ).delete()
        if emp_detail:
...
def del_board(request, bid):
    try:
        board_detail = Board.objects.filter(
            id=bid, o_id_id=request.session["o_id"]
        ).delete()
        if board_detail:
...
def del_proj(request, pid):
    try:
        project_detail = Project.objects.filter(
            id=pid, o_id_id=request.session["o_id"]
        ).delete()
        if project_detail:
    ...
    if request.method == "POST":
        ...
        moni_details = (
            Monitoring.objects.filter(
                o_id_id=o_id, e_id_id=e_id, m_log_ts__startswith=m_date_f2
            )
            .exclude(m_title="")
            .values()
        )
...
def depth_view_app_web(request):
    if request.method == "POST":
        ...
        md_date_f2 = md_date_f1.strftime("%Y-%m-%d")
        depth_moni_details = (
            MonitoringDetails.objects.filter(
                o_id_id=o_id, e_id_id=e_id, md_date__startswith=md_date_f2
            )
            .exclude(md_title="")
            .values()
        )
...
def user_profile(request):
    if request.method == "GET":
        ...
        count_no_of_completed_tasks = Task.objects.filter(
            o_id_id=o_id, e_id_id=e_id, t_status="completed"
        ).count()
        count_no_of_pending_tasks = (
            count_no_of_total_tasks - count_no_of_completed_tasks
        )
...
def ss_monitoring(request):
    if request.method == "POST":
        ...
        ss_date_f2 = ss_date_f1.strftime("%Y-%-m-%-d")  # For Unix/Linux/macOS
        ss_moni_details = ScreenShotsMonitoring.objects.filter(
            o_id_id=o_id, e_id_id=e_id, ssm_log_ts__startswith=ss_date_f2
        ).values()
...
def user_ss_monitoring(request):
    if request.method == "POST":
        ...
        ss_date_f1 = datetime.datetime.strptime(ss_date, "%Y-%m-%d")
        # Format without zero-padding to match database format
        ss_date_f2 = ss_date_f1.strftime("%Y-%-m-%-d")  # For Unix/Linux/macOS
        ...
        print("EmpViewSSMoniLogs", list(ss_moni_details))
...
def power_monitoring(request):
    if request.method == "POST":
        ...
        pm_date_f1 = datetime.datetime.strptime(pm_date, "%Y-%m-%d")
        pm_date_f2 = pm_date_f1.strftime("%Y-%m-%d")
        ss_power_details = PowerMonitoring.objects.filter(
            o_id_id=o_id, e_id_id=e_id, pm_log_ts__startswith=pm_date_f2
        ).values()
...
def user_power_monitoring(request):
    if request.method == "POST":
        ...
        pm_date_f1 = datetime.datetime.strptime(pm_date, "%Y-%m-%d")
        # Format without zero-padding to match database format
        pm_date_f2 = pm_date_f1.strftime("%Y-%-m-%-d")  # For Unix/Linux/macOS
        ss_power_details = PowerMonitoring.objects.filter(
            o_id_id=o_id, e_id_id=e_id, pm_log_ts__startswith=pm_date_f2
        ).values()
...
def create_wp(request):
    ...
    if request.method == "POST":
        ...
        print(f"Organization ID: {o_id}")
        
        wp_ds = request.POST.get("wp_ds")
        wp_type = request.POST.get("wp_type")
        
        print(f"wp_ds: {wp_ds}, wp_type: {wp_type}")
        
        if not wp_ds or not wp_type:
            messages.error(request, "Please fill all required fields.")
            return render(request, "AddWorkProductivity.html")
        
        try:
            wpObj = WorkProductivityDataset.objects.create(
                w_pds=wp_ds, w_type=wp_type, o_id_id=o_id
            )
            print(f"Created object: {wpObj}")
            
            messages.success(
                request, "Work Productivity Dataset Entry was added successfully!"
            )
...
def edit_meet(request, mid):
    try:
        ...
        current_pid = Meeting.objects.filter(id=mid, o_id_id=o_id).values()[0][
            "p_id_id"
        ]
        current_project_name = Project.objects.filter(
            id=current_pid, o_id_id=o_id
        ).values()[0]["p_name"]
        project_names = Project.objects.filter(o_id_id=o_id).values_list(
            "p_name", flat=True
        )
            ...
            if meet_details:
                ...
                if user_details:
                    for ud in user_details:
                        ...
                        s_name = ud["e_name"]
                        s_email = ud["e_email"]
                        message = f"Hi, {s_name} New Meeting for created for you! Details are Meeting Name :{meet_name}, Meeting Description: {meet_desc}, Date Time: {start_date} {start_time} Check on EmployeeTrackingDesk !"
                        email_from = settings.EMAIL_HOST_USER
                        recipient_list = [s_email]
                        send_mail(subject, message, email_from, recipient_list)
        ...
        return render(
            ...
            "UpdateMeeting.html",
            {
                "meet_details": meet_details[0],
                "zipped_pid_pnames": zipped_pid_pnames,
...
def del_meet(request, mid):
    try:
        meet_details = Meeting.objects.filter(
            id=mid, o_id_id=request.session["o_id"]
        ).delete()
        if meet_details:
...
def del_wp(request, wid):
    try:
        wpds_details = WorkProductivityDataset.objects.filter(
            id=wid, o_id_id=request.session["o_id"]
        ).delete()
        if wpds_details:
            messages.success(
                request, "Work Productivity Dataset Entry was deleted successfully!"
            )
...
def create_task(request):
    ...
    if request.method == "POST":
        ...
        if taskObj:
            ...
            subject = "EmployeeTrackingDesk - New Task Created for you"
            message = f'Hi {empDetails[0]["e_name"]} , Your organization as created a new task : {t_name} , description : {t_desc}, priority : {t_priority} and deadline for task is : {t_deadline_date}, Login in your account to get more information. From: EmployeeTrackingDesk. '
            email_from = settings.EMAIL_HOST_USER
            recipient_list = [
                empDetails[0]["e_email"],
            ]
            send_mail(subject, message, email_from, recipient_list)
...
def update_task(request, pk):
    try:
        ...
        p_id = tasks.p_id_id
        projects_emp_link = Project_Employee_Linker.objects.filter(
            o_id_id=o_id, p_id_id=p_id
        ).all()
        if request.method == "POST":
            t_name = request.POST["t_name"]
            ...
            if tasks:
                empDetails = Employee.objects.filter(id=e_id, o_id_id=o_id).values()
                subject = "EmployeeTrackingDesk - Task Updated for you"
                message = f'Hi {empDetails[0]["e_name"]} , Your organization as updated a task : {t_name} , description : {t_desc}, priority : {t_priority} and deadline for task is : {t_deadline_date}, Login in your account to get more information. From: EmployeeTrackingDesk. '
                email_from = settings.EMAIL_HOST_USER
                recipient_list = [
                    empDetails[0]["e_email"],
                ]
            ...
            if tasks:
                return render(
                    request,
                    "UpdateTask.html",
                    {"tasks": tasks, "projects_emp_link": projects_emp_link},
                )
            else:
                messages.error(request, "Some Error was occurred!")
                return HttpResponseRedirect("/read-task")
...
def get_emps_not_in_project(request, pid):
    eids = list(
        Project_Employee_Linker.objects.filter(
            p_id_id=pid, o_id_id=request.session["o_id"]
        ).values_list("e_id_id", flat=True)
    )
    all_eids = list(
        Employee.objects.filter(o_id_id=request.session["o_id"]).values_list(
            "id", flat=True
        )
    )
    unsassigned_emp_ids = list(set(all_eids) - set(eids)) + list(
        set(all_eids) - set(eids)
    )
    unassigned_emp_enames = [
        Employee.objects.filter(id=eid, o_id_id=request.session["o_id"]).values_list(
            "e_name", flat=True
        )[0]
        for eid in unsassigned_emp_ids
    ]
...
def get_emps_by_project(request, pid):
    eids = list(
        Project_Employee_Linker.objects.filter(p_id=pid).values_list("e_id", flat=True)
    )
    enames = [
        Employee.objects.filter(id=eid).values_list("e_name", flat=True)[0]
        for eid in eids
    ]
...
def overview_task(request):
    o_id = request.session["o_id"]
    board_details = Board.objects.filter(o_id_id=o_id).all()
    org_board_names = list(
        Board.objects.filter(o_id_id=o_id).values_list("b_name", flat=True)
    )
    if request.method == "POST":
    ...
    try:
        ...
        if request.method == "POST":
            ...
            attendance_logs = list(
                AttendanceLogs.objects.filter(
                    o_id_id=o_id, e_id_id=e_id, a_date=m_date
                ).values_list(
                    "a_date", "a_ip_address", "a_time_zone", "a_lat", "a_long"
                )
            )[0]
            logged_in_time = list(
                AttendanceLogs.objects.filter(
                    o_id_id=o_id, e_id_id=e_id, a_date=m_date, a_status=1
                ).values_list("a_time")
            )[0][0]
            logged_out_time = list(
                AttendanceLogs.objects.filter(
                    o_id_id=o_id, e_id_id=e_id, a_date=m_date, a_status=0
                ).values_list("a_time")
            )[0][0]
            logged_in_time = datetime.datetime.fromtimestamp(
                int(logged_in_time)
            ).strftime("%H:%M:%S")
            logged_out_time = datetime.datetime.fromtimestamp(
                int(logged_out_time)
            ).strftime("%H:%M:%S")
            total_time_logged = datetime.datetime.strptime(
                logged_out_time, "%H:%M:%S"
            ) - datetime.datetime.strptime(logged_in_time, "%H:%M:%S")
            context = {
                "msg": emp_details,
...
def user_view_attendance(request):
    ...
    if request.method == "POST":
        ...
        m_date = datetime.datetime.strftime(m_date, "%Y-%#m-%#d")
        attendance_logs = list(
            AttendanceLogs.objects.filter(e_id=e_id, a_date=m_date).values_list(
                "a_date", "a_ip_address", "a_time_zone", "a_lat", "a_long"
            )
        )[0]
        logged_in_time = list(
            AttendanceLogs.objects.filter(
                e_id=e_id, a_date=m_date, a_status=1
            ).values_list("a_time")
        )[0][0]
        logged_out_time = list(
            AttendanceLogs.objects.filter(
                e_id=e_id, a_date=m_date, a_status=0
            ).values_list("a_time")
        )[0][0]
        logged_in_time = datetime.datetime.fromtimestamp(int(logged_in_time)).strftime(
            "%H:%M:%S"
        )
        logged_out_time = datetime.datetime.fromtimestamp(
            int(logged_out_time)
...
def unassign_employee(request):
    ...
    proj_emp_ids = Project_Employee_Linker.objects.filter(
        o_id_id=o_id, p_id_id=pid
    ).values_list("e_id_id", flat=True)
    emp_details = [
        list(Employee.objects.filter(id=eid).values())[0] for eid in proj_emp_ids
    ]
    return render(
        request, "unassignemp.html", {"msg": emp_details, "pname": pname, "pid": pid}
    )
...
def unassign_emp(request, eid):
    if request.method == "POST":
        o_id = request.session["o_id"]
        pid = request.POST["p_id"]
        emp_detail = Project_Employee_Linker.objects.filter(
            o_id_id=o_id, e_id_id=eid, p_id_id=pid
        ).delete()
...
def create_notice(request):
    ...
    if request.method == "POST":
        ...
        noticeObj = OrganizationNews.objects.create(
            on_title=on_title, on_desc=on_desc, o_id_id=o_id
        )
        if noticeObj:
            user_details = Employee.objects.filter(o_id_id=o_id).values()
            if user_details:
                for ud in user_details:
                    subject = "EmployeeTrackingDesk - New Notice Published"
                    s_name = ud["e_name"]
                    s_email = ud["e_email"]
                    message = f"Hi, {s_name} Your Organization has published a new notice, Notice Title: {on_title} Check on EmployeeTrackingDesk !"
                    email_from = settings.EMAIL_HOST_USER
                    recipient_list = [s_email]
                    send_mail(subject, message, email_from, recipient_list)
...
def delete_notice(request, pk):
    try:
        noticeObj = OrganizationNews.objects.get(
            id=pk, o_id_id=request.session["o_id"]
        ).delete()
        if noticeObj:
...
def emp_update_tasks(request, tid):
    ...
    task_details = Task.objects.filter(id=tid, o_id_id=o_id, e_id_id=e_id).update(
        t_update_date=t_update_date, t_status="completed"
    )
    if task_details:
...
def get_work_productivity_details(o_id, e_id, m_date):
    ...
    wp_ds_pr_details_unclean = list(
        WorkProductivityDataset.objects.filter(o_id_id=o_id, w_type="1").values_list(
            "w_pds"
        )
    )
    wp_ds_un_pr_details_unclean = list(
        WorkProductivityDataset.objects.filter(o_id_id=o_id, w_type="0").values_list(
            "w_pds"
        )
    )
    emp_work_data_details_unclean = list(
        MonitoringDetails.objects.filter(o_id_id=o_id, e_id_id=e_id, md_date=md_date_f2)
        .values_list("md_title", "md_total_time_seconds")
        .distinct()
    )

    wp_ds_pr_details = [item for x in wp_ds_pr_details_unclean for item in x]
    wp_ds_un_pr_details = [item for x in wp_ds_un_pr_details_unclean for item in x]
    emp_work_data_details = [item for x in emp_work_data_details_unclean for item in x]
...
def view_project_wise_employees(request):
    pids = list(
        Project.objects.filter(o_id_id=request.session["o_id"]).values_list(
            "id", flat=True
        )
    )
    projEmps = {}
    ...
    for pid in pids:
        e = list(
            Project_Employee_Linker.objects.filter(
                o_id_id=request.session["o_id"], p_id=pid
            ).values_list("e_id", flat=True)
        )
        eids.append(e)
...
def rank_productivity(request):
    ...
    for i in eids:
        print(i)
        all_emps_only_prd_score.append(
            get_only_prod_details(request.session["o_id"], i)
        )
        submission_dates = list(
            Task.objects.filter(o_id=o_id, e_id_id=i).values_list(
                "t_update_date", flat=True
            )
        )
        deadline_dates = list(
            Task.objects.filter(o_id=o_id, e_id_id=i).values_list(
                "t_deadline_date", flat=True
            )
        )
        ...
        for s, d in zip(submission_dates, deadline_dates):
            if s is not None:
                ...
                if str(d_date - s_date).split(" ")[0] == "0:00:00":
                    ...
                    current_emp_taskwise_scores.append(
                        int(str(d_date - s_date).split(" ")[0])
                    )
    ...
    all_emps_total_productivity_score = [
        i * j for i, j in zip(all_emps_taskwise_scores, all_emps_only_prd_score)
    ]
    all_emps_total_productivity_score = [
        round(i / sum(all_emps_total_productivity_score) * 100, 4)
        for i in all_emps_total_productivity_score
    ]
    print(all_emps_taskwise_scores)
    print(all_emps_only_prd_score)
    all_emps_total_productivity_score = dict(
        zip(enames, all_emps_total_productivity_score)
    )
    print(all_emps_total_productivity_score)
    all_emps_only_prd_score_dict = dict(
        sorted(
            all_emps_total_productivity_score.items(),
    ...
    data = [go.Bar(x=emp_name, y=emp_score)]
    fig = go.Figure(data=data)
    plot_div = plot(fig, output_type="div")
    return render(
        request,
        "productivityRanks.html",
        {
            "all_emps_only_prd_score_dict": all_emps_only_prd_score_dict,
            "plot_div": plot_div,
        },
    )
...
def get_overall_work_productivity_details(o_id, e_id):
    ...
    emp_work_data_details_unclean = list(
        MonitoringDetails.objects.filter(o_id_id=o_id, e_id_id=e_id).values_list(
            "md_title", "md_total_time_seconds"
        )
    )
...
def get_emp_logged_in_count_today(request):
    ...
    logged_in_count = AttendanceLogs.objects.filter(
